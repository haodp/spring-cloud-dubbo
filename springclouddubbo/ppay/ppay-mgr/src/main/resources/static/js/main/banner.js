//1：bannerNameSeach --> 修改成自己的查询条件
//2：banner 路径 --> 修改成自己的路径
//3：其他的不用修改，要看明白代码原理
...
app.controller('mainCtrl', function ($scope, $http) {
    ...
    $scope.changePageSize = function (pageSize) {
        $scope.pageNo = 1;
        $scope.getViewDataList();
        resetPage();

    };

    // 修改TODO getViewDataList --> 变换成自己的方法
    // 主画面的一览List取得,初期化以及查询按钮
    $scope.getViewDataList = function () {
        ...
        $http({
        	...
        	// 修改TODO getMenu --> getViewDataList
        	console.log('bannerList.js的BannerType: 执行错误  ');
        });
    };
